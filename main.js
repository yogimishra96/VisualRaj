...
var width = 580 * 1.2, height = 550 * 1.2;
...
d3.json("topojson/rajasthan_districts.topojson", function (error, topology) { // <-A
    ...
    d3.csv("by_year_dist.csv", function (dist_data) {
        ...
        function update_choropleth(year) {
            ...
            var sum_t_female = t_female.reduce((a, b) => a + b, 0);
            var sum_t_male = t_male.reduce((a, b) => a + b, 0);
            var total_male_female = sum_t_female + sum_t_male;

            interp_text_transit("#n_inc", 1000, total_male_female)
            interp_text_transit("#total_female", 1000, sum_t_female)
            interp_text_transit("#total_male", 1000, sum_t_male)
